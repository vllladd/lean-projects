...
theorem card_grid_diff_le_1_of_subsingleton_pw_0_a_turn {g : Game} {n}
...
theorem card_grid_diff_le_1_of_state'₀_0_a_turn {g : Game} {n}
(h :  g.toState' = state'₀ 0) (ht : g.a_turn) :
(Set.univ \ (g.play n).grid).Subsingleton := by
  apply card_grid_diff_le_1_of_subsingleton_pw_0_a_turn <;> simp [h, ht]

theorem card_grid_diff_le_1_of_state'₀_0_not_a_turn {g : Game} {n}
(h :  g.toState' = state'₀ 0) (ht : ¬g.a_turn) :
(Set.univ \ (g.play n).grid).Subsingleton := by
  by_cases he : g.ended
  · simp [Game.move, he, game_play_eq_of_ended he, h]
  cases n; simp [h]; nm n
  rw [game_play_succ']; apply card_grid_diff_le_1_of_subsingleton_pw_0_a_turn
  · simp [h]
...
theorem card_grid_diff_le_1_of_state'₀_0 {g : Game} {n}
(h :  g.toState' = state'₀ 0) :
(Set.univ \ (g.play n).grid).Subsingleton := by
  by_cases h₁ : g.a_turn
  exact card_grid_diff_le_1_of_state'₀_0_a_turn h h₁
  exact card_grid_diff_le_1_of_state'₀_0_not_a_turn h h₁
...
¬(∀ (s₀ s : State'), s₀.Reachable' s → s₀.reachable s) := by
  ...
  simp [h₁]; exact card_grid_diff_le_1_of_state'₀_0 h₁