...
theorem of_a_ap_eq_some {a : A_strat} {s s₁} :
a.1.f s = some s₁ → s.state.a_move s₁ := of_strat_ap_eq_some

theorem of_d_ap_eq_some {d : D_strat} {s s₁} :
d.1.f s = some s₁ → s.state.d_move s₁ := of_strat_ap_eq_some

theorem of_a_ap_eq_some' {a : A_strat} {s s₁}
(h : a.1.f s = some s₁) : s.state.a_has_move := ⟨s₁, of_a_ap_eq_some h⟩

theorem of_d_ap_eq_some' {d : D_strat} {s s₁}
(h : d.1.f s = some s₁) : s.state.d_has_move := ⟨s₁, of_d_ap_eq_some h⟩
...
@[simp]
theorem state'₀_a {pw} : (state'₀ pw).a = ⟨0, 0⟩ := rfl
...
theorem game_play_pw_eq {g : Game} {n} : (g.play n).pw = g.pw := by
  apply invariant_play_of_invariant_move Game.pw; simp

@[simp]
theorem game_state_pw_play_eq {g : Game} {n} : (g.play n).state.pw = g.pw :=
  game_play_pw_eq
...
g.state'.valid := state'_valid_of_state_valid # state_valid_of_game_valid h
...
theorem d_always_has_move' {g : Game} (h : g.valid) : g.state'.d_has_move := by
