import Projects.Util.Data.Trie.Equiv
...
theorem option_map_ofRaw_get1?_eq_of_equiv {a b : Raw α β} {k}
(h₁ : a.Equiv b) : Option.map ofRaw (a.get1? k) = Option.map ofRaw (b.get1? k) := by
  ext c
  rcases c with ⟨c⟩
  simp
...
def get1? (t : Trie α β) (k : α) : Option (Trie α β) :=
  t.inner.lift (λ t => t.get1? k |>.map ofRaw) # λ _ _ => option_map_ofRaw_get1?_eq_of_equiv

theorem depth_lt {k} (h : t.get1? k = some t') : t'.depth < t.depth := by
  rcases t, t' with ⟨⟨t⟩, ⟨t'⟩⟩
  revert h
...
def eqComp [hb : DecidableEq β] (t₁ t₂ : Trie α β) : Bool :=
  t₁.inner.liftOn₂ t₂.1 Raw.equivComp Raw.equivComp_eq_of_equiv2_expl

theorem eq_iff_eqComp [hb : DecidableEq β] : t₁ = t₂ ↔ t₁.eqComp t₂ := by
  rcases t₁, t₂ with ⟨⟨t⟩, ⟨t'⟩⟩
  induction t, t' using Quotient.inductionOn₂
  simp [eqComp]; rfl

instance [hb : DecidableEq β] : DecidableEq (Trie α β) :=
  λ _ _ => decidable_of_bool _ eq_iff_eqComp.symm

@[simp]
theorem eqComp_iff_decide_eq [hb : DecidableEq β] : t₁.eqComp t₂ ↔ decide (t₁ = t₂) := by
  simp [eq_iff_eqComp]
...
-- #check 0 #exit
-- 
-- protected def insert (t : Trie α β) (is : List α) (x : β) : Trie α β :=
-- 
-- #check 0 #exit
-- 
...
-- 
-- theorem mem_iff_get?_eq_some {i : α} : i ∈ mp ↔ ∃ x, mp.get1? i = some x :=
--   Std.ExtDHashMap.mem_iff_get?_eq_some
-- 
-- theorem get?_eq_some_of_mem {i : α} (h : i ∈ mp) : ∃ x, mp.get1? i = some x :=
--   Std.ExtDHashMap.get1?_eq_some_of_mem h
-- 
-- theorem get?_eq_none_of_not_mem {i : α} (h : i ∉ mp) : mp.get1? i = none :=
--   Std.ExtDHashMap.get1?_eq_none_of_not_mem h
-- 
-- @[simp]
-- theorem get?_eq_none_iff {i} : mp.get1? i = none ↔ i ∉ mp :=
--   Std.ExtDHashMap.get1?_eq_none_iff
-- 
-- theorem get?_map {f : α → β → γ} {i : α} :
-- (mp.map f).get1? i = (mp.get1? i).map (f i) :=
--   Std.ExtDHashMap.get1?_map
-- 
...
--   rwa [ext_iff']
-- 
-- theorem ext_iff {m₁ m₂ : Map α β} : m₁ = m₂ ↔ ∀ i, m₁.get1? i = m₂.get1? i := by
--   rcases m₁ with ⟨m₁⟩; rcases m₂ with ⟨m₂⟩; simp
--   exact Std.ExtDHashMap.ext_iff
-- 
-- @[ext]
-- theorem ext {m₁ m₂ : Map α β} (h : ∀ i, m₁.get1? i = m₂.get1? i) : m₁ = m₂ := by
--   rwa [ext_iff]
-- 
-- theorem get?_eq_ite_of_unit {m : Map α Unit} {i} :
-- m.get1? i = if i ∈ m then some () else none :=
--   Std.ExtDHashMap.get1?_eq_ite_of_unit
-- 
-- @[simp]
-- theorem get?_empty {i} : (∅ : Map α β).get1? i = none :=
--   Std.ExtDHashMap.get1?_empty
-- 
...
-- 
-- @[simp]
-- theorem mem_toList [LinearOrder α] {x} : x ∈ mp.toList ↔ mp.get1? x.1 = x.2 := by
--   rcases mp with ⟨⟨mp⟩⟩; unfold toList
--   apply mp.ind
--   clear mp; intro mp
--   unfold toSortedList get? Std.ExtDHashMap.get1?
--   rcases x with ⟨x, y⟩
...
-- mp.get1? i = if i ∈ mp then some # mp.get! i else none :=
--   mp.1.get1?_eq_ite
-- 
-- @[simp]
...
-- 
-- theorem get!_eq_get?_get! [Inhabited β] {i} : mp.get! i = (mp.get1? i).get! :=
--   Std.ExtDHashMap.get!_eq_get!_get?
-- 
-- @[simp]
-- theorem get?_modify {i} {f : β → β} {j} :
-- (mp.modify i f).get1? j = if j = i then
-- (mp.get1? j).map f else mp.get1? j := by
--   split_ifs with h₁; subst h₁; simp [modify, get?]
--   simp [modify, get?, Std.ExtDHashMap.get1?_modify, ne_symm' h₁]
-- 
-- @[simp]
-- theorem mem_values [LinearOrder α] {x} : x ∈ mp.values ↔ ∃ i, mp.get1? i = some x := by
--   simp [values]
-- 
...
-- 
-- theorem mem_of_get?_eq_some {i x} (h : mp.get1? i = some x) : i ∈ mp := by
--   simp [mem_iff_get?_eq_some, h]
-- 
-- theorem get!_eq_get!_get? {i} [hb : Inhabited β] :
-- mp.get! i = (mp.get1? i).get! := Std.ExtDHashMap.get!_eq_get!_get?
-- 
-- @[simp]
-- theorem get?_eq_some_get!_iff {i} [hb : Inhabited β] :
-- mp.get1? i = some (mp.get! i) ↔ i ∈ mp := by
--   simp [get?_eq_ite]
-- 
-- @[simp]
-- theorem get?_eq_some_get?_get! {i} [hb : Inhabited β] :
-- mp.get1? i = some (mp.get1? i).get! ↔ i ∈ mp := by
...
-- mp.count p = 0 ↔ ∀ i x, mp.get1? i = some x → ¬p i x :=
...
--   mp.insert i # (mp.get1? i).getD 0 + 1
-- 
-- theorem get?_insert {i j x} :
-- (mp.insert j x).get1? i = if j = i then some x else mp.get1? i := by
--   convert mp.1.get1?_insert; simp; rfl
-- 
