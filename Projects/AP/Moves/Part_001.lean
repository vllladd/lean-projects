...
s₁ ∈ s.simStates st ↔ s.ReachableVia st s₁ := by rfl
...
∃ s₂, sys.tr s₁ (st.a.f s₁) = some s₂ ∧ st.a.f s₁ = p := by
  ...
  · rintro ⟨h₁, s₂, s₃, h₂, h₃⟩
    ...
    rcases r with ⟨s₁, r⟩
    rcases h₁ with ⟨n, h₁⟩
    simp [h₁, diff_eq_of_simulate h₁] at hr
    rcases hr with ⟨rfl, rfl⟩
    simp at h₂ ⊢; simp [h₂]; use n
...
theorem State.diff_init_succ {s} [hs : sys.WF s] : s.init.diff s + 1 = s.hist.length := by
  simp [init, diff]

@[simp]
theorem State.diff_init {s} [hs : sys.WF s] : s.init.diff s = s.hist.length - 1 := by
...
@[simp]
theorem State.diffTrs_init {s} [hs : sys.WF s] : s.init.diffTrs s = s.hist.init.reverse := by
  simp [diffTrs]
...
theorem State.mem_simStatesRangeAuxVia_iff {s s₁ s' : State} {st r n} [hr : DecidableRel r]
(h : sys.simulate st.f s n = (s₁, 0)) : s' ∈ s.simStatesRangeAuxVia r st s₁ ↔
...
s₃.hist.length ≤ s₁.hist.length ∧ r s₃.hist.length s₁.hist.length)) := by
  ...
  simp [simStatesRangeAuxVia, ReachableVia, ReachableVia', h', diff_eq_of_simulate_full h]
  constructor
  · rintro ⟨⟨k, hk, h₂⟩, h₃⟩
...
theorem State.mem_simStatesIccVia_iff_of {s s₁ s₂ : State} {st : Strat} {n}
(h : sys.simulate st.f s n = (s₁, 0)) : s₂ ∈ s.simStatesIccVia st s₁ ↔
∃ k, k ≤ n ∧ sys.simulate st.f s k = (s₂, 0) := by
  simp [simStatesIccVia, mem_simStatesRangeAuxVia_iff h]; constructor
...
theorem State.mem_simStatesIcoVia_iff_of {s s₁ s₂ : State} {st : Strat} {n}
(h : sys.simulate st.f s n = (s₁, 0)) : s₂ ∈ s.simStatesIcoVia st s₁ ↔
∃ k, k < n ∧ sys.simulate st.f s k = (s₂, 0) := by
  simp [simStatesIcoVia, ReachableVia, mem_simStatesRangeAuxVia_iff h]; constructor
  · rintro ⟨⟨k, h₁⟩, h₂⟩; use k; simp [h₁]; have := length_hist_eq_of_simulate_eq h
    have := length_hist_eq_of_simulate_eq h₁; omega
  · rintro ⟨k, h₁, h₂⟩; use by use k;; have := length_hist_eq_of_simulate_eq h
    have := length_hist_eq_of_simulate_eq h₂; omega

theorem State.mem_aSimStatesIccVia_iff_of {s s₁ s₂ : State} {st : Strat} {n} [hs : sys.WF s]
(h : sys.simulate st.f s n = (s₁, 0)) : s₂ ∈ s.aSimStatesIccVia st s₁ ↔ AState s₂ ∧
∃ k, k ≤ n ∧ sys.simulate st.f s k = (s₂, 0) := by
  simp [aSimStatesIccVia, mem_simStatesIccVia_iff_of h]
...
theorem State.mem_aSimStatesIcoVia_iff_of {s s₁ s₂ : State} {st : Strat} {n} [hs : sys.WF s]
(h : sys.simulate st.f s n = (s₁, 0)) : s₂ ∈ s.aSimStatesIcoVia st s₁ ↔ AState s₂ ∧
∃ k, k < n ∧ sys.simulate st.f s k = (s₂, 0) := by
  simp [aSimStatesIcoVia, mem_simStatesIcoVia_iff_of h]
  rw [and_comm]; constructor
  · rintro ⟨h₁, k, hk, h₂⟩
    have hs₂ : sys.WF s₂; grind
    rw [aTurn_iff_aState] at h₁; tauto
...
theorem State.mem_simStatesIccVia_iff {s s₁ s₂ : State} {st : Strat} [hs : sys.WF s] :
s₁ ∈ s.simStatesIccVia st s₂ ↔ ∃ k n, k ≤ n ∧ sys.simulate st.f s k = (s₁, 0) ∧
sys.simulate st.f s n = (s₂, 0) := by
  by_cases h₁ : ∃ n, sys.simulate st.f s n = (s₂, 0)
  · choose n h₁ using h₁
    rw [mem_simStatesIccVia_iff_of h₁]
    constructor
    · rintro ⟨k, hk, h₂⟩
      use k, n
    · rintro ⟨k, n', hk, h₂, h₃⟩
...
theorem State.mem_simStatesIcoVia_iff {s s₁ s₂ : State} {st : Strat} [hs : sys.WF s] :
s₁ ∈ s.simStatesIcoVia st s₂ ↔ ∃ k n, k < n ∧ sys.simulate st.f s k = (s₁, 0) ∧
sys.simulate st.f s n = (s₂, 0) := by
  by_cases h₁ : ∃ n, sys.simulate st.f s n = (s₂, 0)
  · choose n h₁ using h₁
    rw [mem_simStatesIcoVia_iff_of h₁]
    constructor
    · rintro ⟨k, hk, h₂⟩
      use k, n
    · rintro ⟨k, n', hk, h₂, h₃⟩
...
theorem State.mem_aSimStatesIccVia_iff {s s₁ s₂ : State} {st : Strat} [hs : sys.WF s] :
s₁ ∈ s.aSimStatesIccVia st s₂ ↔ AState s₁ ∧ ∃ k n, k ≤ n ∧ sys.simulate st.f s k = (s₁, 0) ∧
sys.simulate st.f s n = (s₂, 0) := by
  simp [aSimStatesIccVia, mem_simStatesIccVia_iff]; rw [and_comm]; constructor
  · rintro ⟨ht, k, n, hk, h₁, h₂⟩; have hs₁ : sys.WF s₁; grind
    rw [aTurn_iff_aState] at ht; use ht, k, n
  · rintro ⟨hs₁, h⟩; simpa

theorem State.mem_aSimStatesIcoVia_iff {s s₁ s₂ : State} {st : Strat} [hs : sys.WF s] :
s₁ ∈ s.aSimStatesIcoVia st s₂ ↔ AState s₁ ∧ ∃ k n, k < n ∧ sys.simulate st.f s k = (s₁, 0) ∧
sys.simulate st.f s n = (s₂, 0) := by
  simp [aSimStatesIcoVia, mem_simStatesIcoVia_iff]; rw [and_comm]; constructor
  · rintro ⟨ht, k, n, hk, h₁, h₂⟩; have hs₁ : sys.WF s₁; grind
    rw [aTurn_iff_aState] at ht; use ht, k, n
  · rintro ⟨hs₁, h⟩; simpa
...
@[simp]
theorem State.mem_simStatesIccVia_self_iff {s s' : State} {st} [hs : sys.WF s] :
s' ∈ s.simStatesIccVia st s ↔ s' = s := by
  simp [mem_simStatesIccVia_iff]
...
theorem State.mem_aVisitedIcc_of_mem_simStatesIccVia {s s' s₁ : State} {st : Strat} {n}
[hs : sys.WF s] (h₁ : sys.simulate st.f s n = (s₁, 0))
(h₂ : s' ∈ s.simStatesIccVia st s₁) : s'.aPos ∈ s.aVisitedIcc s₁ := by
  rw [mem_simStatesIccVia_iff_of h₁] at h₂; choose k hk h₂ using h₂
...
theorem State.mem_aVisitedIcc_iff_mem_simStatesIccVia {s s₁ : State} {st : Strat} {n p}
[hs : sys.WF s] (h : sys.simulate st.f s n = (s₁, 0)) :
p ∈ s.aVisitedIcc s₁ ↔ ∃ s' ∈ s.simStatesIccVia st s₁, s'.aPos = p := by
  symm; constructor; rintro ⟨s', h₁, rfl⟩; exact mem_aVisitedIcc_of_mem_simStatesIccVia h h₁
  rw [mem_aVisitedIcc_iff_of h]; intro h₁; simp [mem_simStatesIccVia_iff]
...
theorem State.mem_simStatesIccVia_of_mem_simStatesIcoVia {s s₁ s' : State} {st} [hs : sys.WF s]
(h : s' ∈ s.simStatesIcoVia st s₁) : s' ∈ s.simStatesIccVia st s₁ := by
  rw [mem_simStatesIcoVia_iff] at h; rw [mem_simStatesIccVia_iff]
  choose k n hk h₁ h₂ using h; use k, n, by omega

theorem State.mem_aSimStatesIccVia_of_mem_aSimStatesIcoVia {s s₁ s' : State} {st} [hs : sys.WF s]
(h : s' ∈ s.aSimStatesIcoVia st s₁) : s' ∈ s.aSimStatesIccVia st s₁ := by
  rw [mem_aSimStatesIcoVia_iff] at h; rw [mem_aSimStatesIccVia_iff]
  choose hs' k n hk h₁ h₂ using h; use hs', k, n, by omega

theorem State.simStatesIcoVia_subset_simStatesIccVia {s s₁ : State} {st} [hs : sys.WF s] :
s.simStatesIcoVia st s₁ ⊆ s.simStatesIccVia st s₁ :=
  λ _ => mem_simStatesIccVia_of_mem_simStatesIcoVia

theorem State.aSimStatesIcoVia_subset_aSimStatesIccVia {s s₁ : State} {st} [hs : sys.WF s] :
s.aSimStatesIcoVia st s₁ ⊆ s.aSimStatesIccVia st s₁ :=
  λ _ => mem_aSimStatesIccVia_of_mem_aSimStatesIcoVia

theorem State.simStatesIcoVia_eq_erase_simStatesIccVia {s s₁ : State} {st} [hs : sys.WF s] :
s.simStatesIcoVia st s₁ = (s.simStatesIccVia st s₁).erase s₁ := by
  ext s'; simp [mem_simStatesIcoVia_iff, mem_simStatesIccVia_iff]; constructor
  · rintro ⟨k, n, hk, h₁, h₂⟩
...
theorem State.aSimStatesIcoVia_eq_erase_aSimStatesIccVia {s s₁ : State} {st} [hs : sys.WF s] :
s.aSimStatesIcoVia st s₁ = (s.aSimStatesIccVia st s₁).erase s₁ := by
  unfold aSimStatesIcoVia aSimStatesIccVia
  rw [simStatesIcoVia_eq_erase_simStatesIccVia]
  ext; simp; tauto

theorem State.mem_simStatesIcoVia_mem_simStatesIccVia_and_ne {s s₁ s' : State} {st} [hs : sys.WF s]
(h₁ : s' ∈ s.simStatesIccVia st s₁) (h₂ : s' ≠ s₁) : s' ∈ s.simStatesIcoVia st s₁ := by
  simp [simStatesIcoVia_eq_erase_simStatesIccVia]; tauto

theorem State.mem_aSimStatesIcoVia_of_mem_aSimStatesIccVia_and_ne
{s s₁ s' : State} {st} [hs : sys.WF s]
(h₁ : s' ∈ s.aSimStatesIccVia st s₁) (h₂ : s' ≠ s₁) : s' ∈ s.aSimStatesIcoVia st s₁ := by
  simp [aSimStatesIcoVia_eq_erase_aSimStatesIccVia]; tauto

theorem State.mem_simStatesIccVia_of_mem_aSimStatesIccVia {s s₁ s' : State} {st}
(h : s' ∈ s.aSimStatesIccVia st s₁) : s' ∈ s.simStatesIccVia st s₁ := by
  simp [aSimStatesIccVia] at h; exact h.1

theorem State.mem_simStatesIcoVia_of_mem_aSimStatesIcoVia {s s₁ s' : State} {st}
(h : s' ∈ s.aSimStatesIcoVia st s₁) : s' ∈ s.simStatesIcoVia st s₁ := by
  simp [aSimStatesIcoVia] at h; exact h.1

theorem State.mem_aVisitedIcc_of_mem_aSimStatesIccVia {s s' s₁ : State} {st : Strat} {n}
[hs : sys.WF s] (h₁ : sys.simulate st.f s n = (s₁, 0))
(h₂ : s' ∈ s.aSimStatesIccVia st s₁) : s'.aPos ∈ s.aVisitedIcc s₁ :=
  mem_aVisitedIcc_of_mem_simStatesIccVia h₁ # mem_simStatesIccVia_of_mem_aSimStatesIccVia h₂

theorem State.mem_aVisitedIcc_of_mem_aSimStatesIcoVia {s s' s₁ : State} {st : Strat} {n}
[hs : sys.WF s] (h₁ : sys.simulate st.f s n = (s₁, 0))
(h₂ : s' ∈ s.aSimStatesIcoVia st s₁) : s'.aPos ∈ s.aVisitedIcc s₁ :=
  mem_aVisitedIcc_of_mem_aSimStatesIccVia h₁ # mem_aSimStatesIccVia_of_mem_aSimStatesIcoVia h₂
...
theorem wf_of_mem_simStatesRangeAuxVia {s s₁ s' : State} {st r}
[hr : DecidableRel r] [hs : sys.WF s]
(h : s' ∈ s.simStatesRangeAuxVia r st s₁) : sys.WF s' := by
  simp [State.simStatesRangeAuxVia] at h; split_ifs at h with h₁ <;> simp at h
  obtain ⟨⟨-, -, rfl⟩, -⟩ := h; infer_instance

theorem wf_of_mem_simStatesIccVia {s s₁ s' : State} {st} [hs : sys.WF s]
(h : s' ∈ s.simStatesIccVia st s₁) : sys.WF s' :=
  wf_of_mem_simStatesRangeAuxVia h

theorem wf_of_mem_simStatesIcoVia {s s₁ s' : State} {st} [hs : sys.WF s]
(h : s' ∈ s.simStatesIcoVia st s₁) : sys.WF s' :=
  wf_of_mem_simStatesRangeAuxVia h

theorem State.mem_aVisitedIcc_iff_mem_aSimStatesIccVia {s s₁ : State} {st : Strat} {n p}
[hs : sys.WF s] [hs₁ : AState s₁] (h : sys.simulate st.f s n = (s₁, 0)) :
p ∈ s.aVisitedIcc s₁ ↔ ∃ s' ∈ s.aSimStatesIccVia st s₁, s'.aPos = p := by
  symm; constructor; rintro ⟨s', h₂, rfl⟩; exact mem_aVisitedIcc_of_mem_aSimStatesIccVia h h₂
  intro h₁
  rw [mem_aVisitedIcc_iff_of h] at h₁
  simp [mem_aSimStatesIccVia_iff]
  simp [and_assoc]
...
theorem State.mem_aVisitedIco_of_mem_simStatesIcoVia {s s' s₁ : State} {st : Strat} {n}
[hs : sys.WF s] (h₁ : sys.simulate st.f s n = (s₁, 0))
(h₂ : s' ∈ s.simStatesIcoVia st s₁) : s'.aPos ∈ s.aVisitedIco s₁ := by
  rw [mem_simStatesIcoVia_iff_of h₁] at h₂; choose k hk h₂ using h₂
...
theorem State.mem_aVisitedIco_iff_mem_simStatesIcoVia {s s₁ : State} {st : Strat} {n p}
[hs : sys.WF s] (h : sys.simulate st.f s n = (s₁, 0)) :
p ∈ s.aVisitedIco s₁ ↔ ∃ s' ∈ s.simStatesIcoVia st s₁, s'.aPos = p := by
  symm; constructor; rintro ⟨s', h₁, rfl⟩; exact mem_aVisitedIco_of_mem_simStatesIcoVia h h₁
  rw [mem_aVisitedIco_iff_of h]; intro h₁; simp [mem_simStatesIcoVia_iff]
...
@[simp]
theorem State.simStatesIcoVia_self {s : State} {st} : s.simStatesIcoVia st s = ∅ := by
  simp [simStatesIcoVia, simStatesRangeAuxVia]

@[simp]
theorem State.aSimStatesIcoVia_self {s : State} {st} : s.aSimStatesIcoVia st s = ∅ := by
  simp [aSimStatesIcoVia]

@[simp]
theorem State.aVisitedIco_self {s : State} : s.aVisitedIco s = ∅ := by
  simp [aVisitedIco]

theorem State.simStatesIccVia_eq_of_tr {s s₁} {st : Strat} [hs : sys.WF s]
(h : sys.tr s (st.f s) = some s₁) : s.simStatesIccVia st s₁ = {s, s₁} := by
  have h' : sys.simulate st.f s 1 = (s₁, 0); simpa
  ext s'; simp [simStatesIccVia, mem_simStatesRangeAuxVia_iff h']; constructor
...
theorem State.simStatesIcoVia_eq_of_tr {s s₁} {st : Strat}
(h : sys.tr s (st.f s) = some s₁) : s.simStatesIcoVia st s₁ = {s} := by
  have h' : sys.simulate st.f s 1 = (s₁, 0); simpa
  ext s'; simp [simStatesIcoVia, mem_simStatesRangeAuxVia_iff h']; constructor
  · rintro ⟨⟨n, h₁⟩, h₂⟩; obtain rfl | rfl : n = 0 ∨ n = 1
    · have := length_hist_eq_of_tr h; have := length_hist_eq_of_simulate_eq h₁; omega
    all_goals simp [h] at h₁; grind
  · rintro (rfl | rfl); all_goals simp [length_hist_eq_of_tr h]
...
theorem State.simStatesIcoVia_eq_simStatesIccVia_prev_of {s s₁ : State} {st : Strat} {n}
[hs : sys.WF s] (h₁ : sys.simulate st.f s n = (s₁, 0)) (h₂ : n ≠ 0) :
s.simStatesIcoVia st s₁ = s.simStatesIccVia st s₁.prev := by
  ext s'
  cases n
  · simp at h₂
  nm n
  ...
  rw [prev_eq_of_tr h₃]
  simp [simStatesIcoVia_eq_erase_simStatesIccVia, simStatesIccVia]
  simp [mem_simStatesRangeAuxVia_iff h₁, mem_simStatesRangeAuxVia_iff h₁']
  constructor
  · rintro ⟨h₄, ⟨k, h₅⟩, h₆⟩
    use by use k
    have := length_hist_eq_of_simulate_eq h₁
...
theorem State.aSimStatesIcoVia_eq_aSimStatesIccVia_prev_of {s s₁ : State} {st : Strat} {n}
[hs : sys.WF s] (h₁ : sys.simulate st.f s n = (s₁, 0)) (h₂ : n ≠ 0) :
s.aSimStatesIcoVia st s₁ = s.aSimStatesIccVia st s₁.prev := by
  rw [aSimStatesIcoVia, aSimStatesIccVia, simStatesIcoVia_eq_simStatesIccVia_prev_of h₁ h₂]

theorem State.mem_aVisitedIcoPrev_iff_exi_aSimStatesIcoVia
{s s₁ : State} {st : Strat} {n p} [hs : AState s] [hs₁ : AState s₁] [hst : st.WF]
(h₁ : s ≠ s₁) (h₂ : sys.simulate st.f s n = (s₁, 0)) :
p ∈ s.aVisitedIcoPrev s₁ ↔ ∃ s' ∈ s.aSimStatesIcoVia st s₁, s'.aPos = p := by
  ...
  simp [mem_aSimStatesIcoVia_iff_of h₆, and_assoc]
  constructor
...
s.aNbhdsIcoPrev s₁ ∪ if s = s₁ then ∅ else s₁.prev.aPos.nbhd (s.pw) := by
  ...
  simp at H₆
  choose s₃ H₆ H₈ using H₆
  have hs₃ : AState s₃; grind
  use n, by omega, s₃, by grind, H₆
  rw [DState.aPos_eq_of_tr h₃, AState.aPos_eq_of_tr H₈]