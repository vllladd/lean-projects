...
class State.WF (pw : outParam ℕ) (s : State) : Prop where
  wf : ∃ a d n g, (initGame a d state₀ : Game pw).play n = g ∧
    (g.s = s ∨ ∃ h, (playDMoveAt g h).s = s)

class AState (pw : outParam ℕ) (s : State) extends State.WF pw s where
  ht : s.aTurn = true

class DState (pw : outParam ℕ) (s : State) extends State.WF pw s where
  ht : s.aTurn = false
