import Projects.AP.Defense.Edge
...
instance {p} : Decidable # p ∈ c.points :=
  decidable_of_bool _ mem_points_iff_memPoints.symm
...
(h : c.edge₂.defense.f s = some p) : c.edge₁.defense.f s = none := by
  contrapose! h
  apply ne_of_congr (· = none)
  simp
  rw [Option.ne_none_iff_exists'] at h
  choose p h using h
  apply f_edge₂_eq_none_of_f_edge₁_eq_some H h
...
theorem of_f_eq_some {s p} (h : c.defense.f s = some p) :
c.edge₁.defense.f s = some p ∨ c.edge₂.defense.f s = some p := by
  simp [defense, f] at h; tauto

theorem f_eq_some_iff_of_cnd' {s p} [hs : sys.WF s] (h : c.cnd' s) :
