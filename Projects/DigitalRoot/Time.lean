...
@[simp]
theorem Time.toList_mk {hour minute} :
(⟨hour, minute⟩ : Time).toList = [hour.1, minute.1] := rfl
...
theorem sum_freqTimeMp_eq_24_mul_60 :
(freqTimeMp 10 |>.toList.map (·.snd) |>.sum) = 24 * 60 := by
